Audiobus SDK -- Version 3.0.7 -- May 30 2019
============================================
...
----
 ...
 - Fixed some issues apparent in recent iOS versions due to duplicate Inter-App Audio connection/missing disconnection
   notices, resulting in silent audio for some receivers upon device sample rate change.
 - Fixed an error that could result in silent output for senders on client format change.
 - Fixed notch handling for iPhone Xs Max
 - Introduced new "pipelines" property of ABPort, replacing the now-deprecated audioPipelineIDs and MIDIPipelineIDs properties,
   and fixed some problems resulting in incorrect reporting of pipelines for upstream ports.
 - Added extra measure to prevent IAA buzz errors if app incorrectly stops a hosted audio unit during an audio session interruption
   (see http://devnotes.kymatica.com/iaa_quirks.html#buzz-noise-interruptions-for-iaa-nodes for description of issue and workaround).
 - Fixed bug in state sender mechanism that can result in lost parts
 - Fixed retransmit request handling for large state saves
 - Fixed ‘audiobusConnected’ property for ABMIDIPorts
 - Work around an iOS bug causing spurious audio session interruptions for hosted nodes, which can cause a variety of problems including timeout error conditions for apps with ABAudioReceiverPorts.
 - Added support for very large MIDI packet lists

3.0.6
----

 - Added connectionPanelVisible and connectionPanelFrame properties to ABAudiobusController
 - Added ABMIDIPortFlushOutput function
 - Added MIDIFlushEventBlock properties for ABMIDIReceiverPort and ABMIDIFilterPort for handling MIDI flush events
 - Deprecated 'memberOfActiveAudiobusSession' property of ABAudiobusController which is no longer needed; apps should
   stop their audio engines when disconnected, and don't need to remain active for the duration of an Audiobus session.
 - Fixed an issue with ABAudioReceiverPort timestamps which could cause incorrectly-reported latency
 - Fixed issue where connection panel could vanish after returning to foreground
 - Fixed issue with incorrectly-positioned connection panel when dragging out after position change
 - Fixed issue that could cause apps to remain active in background
 - Significant internal changes to improve reliability and facilitate some changes within the Audiobus app itself

3.0.5
----

 - Fixed issue where showInterAppAudioTransportPanelBlock was not called when apps were loaded in an IAA host app
 - Added workaround for Security framework deadlock bug
 - Fixed issue with ABAudioSenderPortSend when running at other than hardware sample rate
 - Fixed a rare Bonjour-related crash
